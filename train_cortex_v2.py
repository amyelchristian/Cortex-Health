#!/usr/bin/env python3
"""
CORTEX ML MODEL v2.0 - Production Training Pipeline
Focus: Fix high-risk recall (target >= 95%) while maintaining 98%+ accuracy.
...
print("=" * 70)
print("       CORTEX ML MODEL v2.0 - PRODUCTION TRAINING PIPELINE")
...
# Aggressive SMOTE: oversample high-risk to 1.5x the majority class
print("\nApplying strategic SMOTE (high-risk boosted)...")
majority_count = y_train.value_counts().max()
# Oversample high-risk aggressively, medium-risk moderately
sampling_strategy = {
    1: int(majority_count),        # Keep low-risk as is
    2: int(majority_count),        # Balance medium-risk
    3: int(majority_count * 1.5),  # Oversample high-risk to 1.5x majority
}
smote = SMOTE(sampling_strategy=sampling_strategy, random_state=42,
              k_neighbors=min(5, y_train.value_counts().min() - 1))
X_train_bal, y_train_bal = smote.fit_resample(X_train, y_train)

print("After strategic SMOTE (training set):")
for level in sorted(y_train_bal.unique()):
    count = (y_train_bal == level).sum()
    total = len(y_train_bal)
    boost = " [BOOSTED]" if level == 3 else ""
    print(f"  {risk_names[level]} Risk ({level}): {count:,} ({count/total*100:.1f}%){boost}")
...
sample_weights = np.ones(len(y_train_bal))
sample_weights[y_train_bal == 3] = 8.0
sample_weights[y_train_bal == 2] = 2.0
...
xgb_train_acc = accuracy_score(y_train_bal, xgb_model.predict(X_train_scaled) + 1)
...
try:
    ...
    lgbm_train_acc = accuracy_score(y_train_bal, lgbm.predict(X_train_scaled))
    ...
    rf2.fit(X_train_scaled, y_train_bal)
    rf2_time = time.time() - t0
    rf2_pred = rf2.predict(X_test_scaled)
    rf2_acc = accuracy_score(y_test, rf2_pred)
    rf2_train_acc = accuracy_score(y_train_bal, rf2.predict(X_train_scaled))
...
{'-'*65}
 ...
 - Balancing method: Strategic SMOTE (High-Risk oversampled to 1.5x majority)
...
print()
print("=" * 65)
print("       CORTEX MODEL v2.0 TRAINING COMPLETE")
print("=" * 65)
print()
